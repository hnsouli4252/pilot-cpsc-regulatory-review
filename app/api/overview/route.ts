...
async function federalRegisterOverview(retrieved:Date,fromDate:string){
 ...
 return {source:"FederalRegister.gov API — official fallback after Regulations.gov rate limit/unavailability",agency:"CPSC",retrievedAt:retrieved.toISOString(),fromDate,toDate:retrieved.toISOString().slice(0,10),totalElements:total,recordsReturned:data.length,complete,docketCount:null,typeAggregation,cadence,latest:records.slice(0,6),coverage:complete?"All Federal Register records reported by this bounded fallback query were returned. Docket count is withheld because listing metadata does not include dependable docket identifiers.":`The Federal Register API reports ${total} records; ${data.length} were returned. Docket and cadence claims are limited accordingly.`};
}
