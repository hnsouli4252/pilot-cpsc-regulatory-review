...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ docketId, source:"Regulations.gov API v4 — comments endpoint", retrievedAt, page:meta.pageNumber||page, pageSize:meta.pageSize||PAGE_SIZE, totalElements:meta.totalElements??comments.length, officialTotalPages:meta.totalPages||1,totalPages:Math.min(meta.totalPages||1,10), hasNextPage:Boolean(meta.hasNextPage)&&page<10, comments, coverageLimit:`Official API reports ${meta.totalElements??comments.length} comments across ${meta.totalPages||1} page(s). This workspace displays 20 per page and exposes at most the first 10 pages; current page ${meta.pageNumber||page} returned ${comments.length}.` }, { headers:{"Cache-Control":"no-store"} });
  } catch {
